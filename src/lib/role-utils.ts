...
export const checkOrganizationRole = async (
  ...
  try {
    ...
    const hasRequiredRole = checkOrganizationRoleHierarchy(
      userRole,
      requiredRole,
    );
...
export const checkProjectRole = async (
  ...
  try {
    ...
    if (projectRole) {
      ...
      const hasRequiredRole = checkProjectRoleHierarchy(
        projectRole,
        requiredRole,
      );
      return {
    ...
    const mappedProjectRole: ProjectRole =
      orgRole === "SUPER_OWNER" || orgRole === "OWNER"
        ? "OWNER"
        : orgRole === "ADMIN"
          ? "EDITOR"
          : "VIEWER";
...
export const requireOrganizationSuperOwner = async (
  userId: number,
  organizationId: number,
) => {
  ...
  try {
    ...
    if (managerRole.role === "SUPER_OWNER") {
      return {
        ...
        error:
          targetRole.role === "SUPER_OWNER"
            ? "Cannot manage other super owners"
            : undefined,
      };
    }
    ...
    if (managerRole.role === "OWNER") {
      return {
        ...
        error:
          targetRole.role === "OWNER" || targetRole.role === "SUPER_OWNER"
            ? "Cannot manage users with equal or higher privileges"
            : undefined,
      };
    }
    ...
    if (managerRole.role === "ADMIN") {
      return {
        ...
        error:
          targetRole.role !== "MEMBER" ? "Can only manage members" : undefined,
      };
    }
